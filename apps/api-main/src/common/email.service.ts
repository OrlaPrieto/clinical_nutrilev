  ...
  async sendPasswordResetEmail(
    ...
    <body style="font-family: 'Inter', -apple-system, sans-serif; background-color: #faf9f6; padding: 20px; color: #444;">
        <div style="max-width: 600px; margin: 0 auto; background-color: white; border-radius: 24px; padding: 40px; box-shadow: 0 10px 25px rgba(255, 65, 248, 0.05);">
            ...
            <h2 style="font-size: 20px; color: #222; margin-bottom: 15px; font-weight: 600;">Restablecer tu contraseña</h2>
            <p style="font-size: 15px; line-height: 1.6; margin-bottom: 25px; color: #555;">
                Hemos recibido una solicitud para restablecer la contraseña de tu cuenta en Nutrilev. Si no la solicitaste, puedes ignorar este correo de manera segura.
            </p>
            
            <div style="text-align: center; margin: 35px 0;">
                <a href="${resetLink}" style="background-color: #ff41f8; color: white; padding: 14px 20px; text-decoration: none; border-radius: 14px; font-weight: 600; font-size: 15px; display: inline-block; width: 100%; max-width: 280px; box-sizing: border-box; text-align: center; box-shadow: 0 4px 10px rgba(255, 65, 248, 0.2);">
            ...
            <div style="background-color: #fdf5ff; border: 1px solid #ffd9fd; border-radius: 12px; padding: 18px; margin-bottom: 20px;">
                <p style="font-size: 13px; margin: 0; line-height: 1.5; color: #b32eb0;">
                    Por seguridad de tu cuenta, este enlace de recuperación es válido únicamente por <strong>24 horas</strong>.
                </p>
            </div>
    ...
    try {
      await axios.post(
        'https://api.resend.com/emails',
        {
          from: `Nutrilev <${this.emailFrom}>`,
          to: [email],
          subject: '🔒 Restablecer tu contraseña de Nutrilev',
          html: htmlContent,
        },
        {
          headers: {
      ...
      const errMsg = error instanceof Error ? error.message : String(error);
      this.logger.error(`Error sending password reset email via Resend: ${errMsg}`);
      if (axios.isAxiosError(error) && error.response) {
        this.logger.error(
          `Resend API Error: ${JSON.stringify(error.response.data)}`,
        );
